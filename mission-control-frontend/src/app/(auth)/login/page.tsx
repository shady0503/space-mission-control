...
export default function SpaceAuthPage() {
  ...
  const [signupForm, setSignupForm] = useState<{
    ...
    email: string;
    password: string;
    enterpriseChoice: 'await_invitation' | 'create_enterprise';
  }>({ 
    username: '', 
    email: '', 
    password: '',
    enterpriseChoice: 'await_invitation'
  });
  ...
  // Show loading state while checking session or if authenticated (during redirect)
  ...
  // Only render the login form if we're definitely not authenticated
          ...
          {authMode === 'signup' && (
            <form onSubmit={handleSignup} className="space-y-5">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-blue-300">Enterprise Selection</label>
                <div className="grid grid-cols-1 gap-3">
                  <div 
                    className={`relative p-4 rounded-lg border transition-all duration-300 ${
                      signupForm.enterpriseChoice === 'await_invitation'
                  ...
                  >
                    <input
                      ...
                      value="await_invitation"
                      checked={signupForm.enterpriseChoice === 'await_invitation'}
                      onChange={(e) => setSignupForm({ ...signupForm, enterpriseChoice: e.target.value as 'await_invitation' })}
                      className="absolute opacity-0"
                    />
                    <label htmlFor="await_invitation" className="flex items-start space-x-3 cursor-pointer">
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-white">Await Invitation</div>
                        <div className="text-sm text-gray-300">Join an existing enterprise when invited by an administrator</div>
                      </div>
                    </label>
                  </div>
                  ...
                  <div 
                    className={`relative p-4 rounded-lg border transition-all duration-300 ${
                      signupForm.enterpriseChoice === 'create_enterprise'
                  ...
                  >
                    <input
                      ...
                      checked={signupForm.enterpriseChoice === 'create_enterprise'}
                      onChange={(e) => setSignupForm({ ...signupForm, enterpriseChoice: e.target.value as 'create_enterprise' })}
                      className="absolute opacity-0"
                    />
                    <label htmlFor="create_enterprise" className="flex items-start space-x-3 cursor-pointer">
                      <div className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center border-2 ${
                        signupForm.enterpriseChoice === 'create_enterprise'
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-white">Create New Enterprise</div>
                        <div className="text-sm text-gray-300">Start your own enterprise as an administrator</div>
                      </div>
                    </label>
                  </div>
                </div>
